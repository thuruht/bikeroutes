  ...
  return (
    <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "14px 12px",
      ...
      <span style={{ flex: "none", width: size, height: size, borderRadius: 12,
        ...
        <img src="/reki_icon.png" alt="Reki the deer" width={size - 10} height={size - 10}
  ...
  return (
    <div style={{ position: "relative" }}>
      <div className="io-row" style={{ borderRadius: 12 }}>
        <span
          className={"io-dot " + dotClass}
          style={dotNumber != null ? {
            display: "grid", placeItems: "center",
            width: 16, height: 16, fontSize: 9, fontWeight: 700, color: "#fff"
          } : null}
        >{dotNumber != null ? dotNumber : ""}</span>
        ...
        {canClear && !busy && (
          <button className="io-clear" title="Remove stop" aria-label="Remove stop"
            ...
            style={{ border: 0, background: "transparent", cursor: "pointer", color: "var(--muted-txt)",
              display: "grid", placeItems: "center", width: 22, height: 22, flex: "none", borderRadius: 6 }}>
        ...
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 50,
          ...
          {list.map((d, i) => (
            <div key={i} onMouseDown={() => { onPick(d); setQ(d.short); setOpen(false); }}
              ...
              <div className="mono" style={{ fontSize: 11, color: "var(--muted-txt)", marginTop: 1,
        ...
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 55,
          ...
          {list.map((d, i) => (
            <div key={i} onMouseDown={() => { onJump(d); setQ(d.short); setOpen(false); }}
              ...
              <div className="mono" style={{ fontSize: 10.5, color: "var(--muted-txt)", marginTop: 1,
                whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.label}</div>
            </div>
          ))}
        </div>
    ...
    <div className="elev">
      ...
      <div className="elev-chart" ref={ref} onMouseMove={move} onMouseLeave={leave}>
        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
          ...
          {tip && (
            <line x1={(tip.left / 100) * W} x2={(tip.left / 100) * W} y1="0" y2={H}
...
function App() {
  ...
  const styleFor = (th) => ({
    ...
    sources: {
      base: {
        ...
        tiles: tileArr(th),
        tileSize: 256,
        attribution: BR.TILES.attribution,
      },
    },
  ...
  const addRouteLayers = useCallback(() => {
    ...
    map.addLayer({
      ...
      paint: {
        "line-color": theme === "dark" ? "#0b0c08" : "#fffdf7",
        "line-width": 9, "line-opacity": 0.9,
      },
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      center: [BR.HOME.lng, BR.HOME.lat],
      zoom: BR.HOME.zoom,
    ...
    map.on("mousemove", (e) =>
      setReadout(r => ({ ...r, coords: `${e.lngLat.lat.toFixed(4)}, ${e.lngLat.lng.toFixed(4)}` }))
    );
  ...
  useEffect(() => {
    ...
    const wingSVG = (w) =>
      `<svg width="${w}" height="${w}" viewBox="0 0 40 40" style="display:block;filter:drop-shadow(0 2px 3px rgba(0,0,0,.5))">` +
      `<g fill="#ff6b1a" stroke="${edge}" stroke-width="1.1" stroke-linejoin="round" stroke-linecap="round">` +
  ...
  useEffect(() => {
    ...
    BR.route(valid, pref).then(res => {
      ...
      if (map && res && res.coords.length) {
        const lngs = res.coords.map(c => c[0]);
        const lats = res.coords.map(c => c[1]);
        map.fitBounds(
  ...
  }, [wps, pref]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const onScrub = useCallback((frac) => {
    ...
    if (!scrubMarker.current) {
      ...
      el.style.cssText =
        ...
        "border:3px solid var(--orange);box-shadow:0 0 0 4px rgba(255,107,26,.25);";
      scrubMarker.current = new maplibregl.Marker({ element: el }).setLngLat(pt).addTo(map);
    } else {
      scrubMarker.current.setLngLat(pt);
    }
  }, [result]);
  ...
  useEffect(() => {
    ...
    if (!turnMarker.current) {
      ...
      el.style.cssText =
        ...
        "border:3px solid var(--paper);box-shadow:0 0 0 4px rgba(255,107,26,.3);";
      turnMarker.current = new maplibregl.Marker({ element: el }).setLngLat(at).addTo(map);
    } else {
      turnMarker.current.setLngLat(at);
    }
  }, [hoverTurn, result]);
  ...
  const exportGPX = () => {
    if (!result) return;
    const pts = result.coords.map(c =>
      `<trkpt lat="${c[1]}" lon="${c[0]}">${c[2] != null ? `<ele>${c[2]}</ele>` : ""}</trkpt>`
    ).join("");
    download(
      `<?xml version="1.0"?><gpx version="1.1" creator="bikeroutes.org"><trk><name>BikeRoutes route</name><trkseg>${pts}</trkseg></trk></gpx>`,
  ...
  const exportKML = () => {
    if (!result) return;
    const line = result.coords.map(c =>
      `${c[0]},${c[1]}${c[2] != null ? "," + c[2] : ""}`
    ).join(" ");
    download(
      `<?xml version="1.0" encoding="UTF-8"?><kml xmlns="http://www.opengis.net/kml/2.2"><Document><name>BikeRoutes route</name><Placemark><name>Route</name><LineString><tessellate>1</tessellate><coordinates>${line}</coordinates></LineString></Placemark></Document></kml>`,
      "route.kml", "application/vnd.google-earth.kml+xml"
    );
  };

  /* ---- waypoint helpers ---- */
  const setWp = (i, d) =>
    setWps(prev => prev.map((w, j) => j === i ? { lng: d.lng, lat: d.lat, label: d.short } : w));
  const appendWp = (d) =>
    setWps(prev => [...prev, { lng: d.lng, lat: d.lat, label: d.short }]);
  const removeWp = (i) =>
    setWps(prev => prev.filter((_, j) => j !== i));
  const reverseWps = () =>
    setWps(prev => [...prev].reverse());
  const jumpTo = (d) => {
    const map = mapObj.current;
    if (map) map.flyTo({ center: [d.lng, d.lat], zoom: Math.max(map.getZoom(), 13), duration: 800 });
  };
  ...
  const srcBadge = result && (
    <span className="mono" style={{
      ...
      background: result.source === "brouter" || result.source === "worker"
        ? "var(--green-soft)" : "var(--orange-soft)",
      color: result.source === "brouter" || result.source === "worker"
        ? "var(--green)" : "var(--orange)",
      fontWeight: 600,
    }}>
      {result.source === "worker" ? "live · Worker"
    ...
    <div className="app">
      ...
      <div className="topbar">
        ...
        <div className="theme-toggle" role="group" aria-label="Map theme">
          <button
            ...
            title="Daylight" aria-label="Daylight theme"
          >{Ic.sun}</button>
          <button
            className={theme === "dark" ? "active" : ""}
            onClick={() => setTheme("dark")}
            title="Tactical dark" aria-label="Dark theme"
          >{Ic.moon}</button>
        </div>

        <button className="pillbtn solid" onClick={() => setWps([])} title="Clear route">
          {Ic.plus}<span className="btn-label"> New route</span>
        </button>
      </div>
      ...
      <div className="panel">
        <div className="modetabs">
          <button className="active">Plan a route</button>
        </div>
        <div className="panel-scroll">
          ...
          <div className="io" style={{ position: "relative", display: "flex", flexDirection: "column", gap: 8 }}>
            {wps.map((w, i) => (
              <GeoInput
                key={i}
                dotClass={i === 0 ? "a" : i === wps.length - 1 ? "b" : "via"}
                dotNumber={i !== 0 && i !== wps.length - 1 ? i : null}
                value={w.label}
                placeholder={i === 0 ? "Start" : "Stop"}
                onPick={(d) => setWp(i, d)}
                canClear
                onClear={() => removeWp(i)}
              />
            ...
            <GeoInput
              dotClass={wps.length === 0 ? "a" : "add"}
              value=""
              placeholder={
                wps.length === 0
                  ? "Choose start — or click the map"
                  : wps.length === 1
                  ? "Choose destination"
                  : "Add another stop"
              }
            ...
            {valid.length >= 2 && (
              <button
                className="pillbtn"
                onClick={reverseWps}
                style={{ alignSelf: "flex-start", padding: "7px 11px", fontSize: 12 }}
              >{Ic.swap} Reverse</button>
            )}
          </div>
          ...
          {valid.length < 2 ? (
            ...
            <>
              <div className="summary">
                ...
                <div className="stat-grid">
                  <div className="stat">
                    ...
                    <div className="v mono">+{Math.round(result.ascend)}<small> m</small></div>
                  </div>
                  <div className="stat">
                ...
                <div className="diffbar">
                  <div className="dots">
                    {[1, 2, 3].map(n => <i key={n} className={n <= diff ? "on" : ""} />)}
                  </div>
                  <span className="txt">{diffTxt} · {diffSub}</span>
                </div>
                <div className="surface">
                  <div className="bar">
                    <div className="paved" style={{ width: result.surface.paved + "%" }} />
                    <div className="gravel" style={{ width: result.surface.gravel + "%" }} />
                  </div>
              ...
              <div className="actions" style={{ flexWrap: "wrap" }}>
                <button className={saved ? "primary" : ""} onClick={() => setSaved(s => !s)}>
                  {Ic.save}{saved ? "Saved" : "Save"}
                </button>
              ...
              {result.source === "estimated" && (
                <div style={{ marginTop: 14 }}><Reki mood="estimate" /></div>
              )}
                ...
                {result.turns.map((t, i) => (
                  <div key={i} className="turn"
                    ...
                    <div className="body">
                      <div className="road">
                        {(t.type === "start" ? "Depart"
                          : t.type === "arrive" ? "Arrive at destination"
                          : t.type === "left" ? "Turn left"
                          : "Turn right") +
                          ((t.road && t.type !== "start" && t.type !== "arrive" &&
                            !["Continue", "Start", "Arrive at destination"].includes(t.road))
                            ? ` onto ${t.road}` : "")}
                      </div>
                      {t.dist > 0 && (
                        <div className="meta mono">{(t.dist / 1000).toFixed(1)} km</div>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <div className="mapctl">
        <div className="zoomstack">
          <button title="Zoom in" aria-label="Zoom in"
            onClick={() => mapObj.current && mapObj.current.zoomIn()}>+</button>
          <button title="Zoom out" aria-label="Zoom out"
            onClick={() => mapObj.current && mapObj.current.zoomOut()}>−</button>
        </div>
      </div>
