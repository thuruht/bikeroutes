...
(function () {
  ...
  // Accepts an array of waypoints [{lng,lat}, ...] (>=2), or legacy (a, b, pref).
  BR.route = async function (points, pref) {
    // legacy 2-arg shim: BR.route(a, b, pref)
    if (!Array.isArray(points)) return BR.route([points, pref], arguments[2]);
    ...
    try {
      ...
      return estimate(pts); // @SEAM fallback so the product never dead-ends
    }
  };
  ...
  function estimate(pts) {
    ...
    const dist = pathLen(coords) * 1.32; // detour factor
    return {
  ...
  // point [lng,lat] at a fraction (0..1) of total length along coords — for elevation scrub
  BR.pointAtFrac = function (coords, frac) {
